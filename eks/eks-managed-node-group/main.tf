...
resource "aws_launch_template" "this" {
  ...
  dynamic "metadata_options" {
    ...
    content {
      ...
      # https://github.com/terraform-aws-modules/terraform-aws-eks/issues/1977
...
resource "aws_eks_node_group" "this" {
  ...
  # Optional
  node_group_name        = var.use_name_prefix ? null : "${var.name}-${random_id.random.b64_url}"
  node_group_name_prefix = var.use_name_prefix ? "${var.name}-${random_id.random.b64_url}-" : null
...
resource "random_id" "random" {
  byte_length = 5
  keepers = {
      eks_node_group_instance_types = jsonencode(var.instance_types),
      eks_node_group_ami_type = var.ami_type,
      eks_node_group_ami_release_version = var.ami_release_version,
      eks_node_group_disk_size = var.block_device_mappings["xvda"]["ebs"]["volume_size"],
      eks_node_group_subnet_list = var.subnet_ids[0]
  }
}
