...
data "aws_iam_policy_document" "access_log_delivery" {
  count = var.create_bucket && var.attach_access_log_delivery_policy ? 1 : 0
...
data "aws_iam_policy_document" "deny_incorrect_encryption_headers" {
  count = var.create_bucket && var.attach_deny_incorrect_encryption_headers ? 1 : 0
...
data "aws_iam_policy_document" "deny_incorrect_kms_key_sse" {
  count = var.create_bucket && var.attach_deny_incorrect_kms_key_sse ? 1 : 0
...
data "aws_iam_policy_document" "deny_unencrypted_object_uploads" {
  count = var.create_bucket && var.attach_deny_unencrypted_object_uploads ? 1 : 0
...
resource "aws_s3_bucket_intelligent_tiering_configuration" "this" {
  for_each = { for k, v in local.intelligent_tiering : k => v if var.create_bucket }
...
resource "aws_s3_bucket_metric" "this" {
  for_each = { for k, v in local.metric_configuration : k => v if var.create_bucket }
...
resource "aws_s3_bucket_inventory" "this" {
  for_each = { for k, v in var.inventory_configuration : k => v if var.create_bucket }
...
data "aws_iam_policy_document" "inventory_and_analytics_destination_policy" {
  count = var.create_bucket && var.attach_inventory_destination_policy || var.attach_analytics_destination_policy ? 1 : 0
